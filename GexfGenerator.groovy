...
class GexfGenerator {
    Boolean mentions, retweets, replies, trim, twint, tweep, flow
    Integer edgeWeight
    File outputFile
//    Boolean isCLI
//    String esQuery

    DateTimeFormatter CREATED_AT_FORMAT = DateTimeFormat.forPattern("EEE MMM dd HH:mm:ss Z yyyy").withZoneUTC().withLocale(Locale.ENGLISH);
    DateTimeFormatter TWEEP_DATE_FORMAT = DateTimeFormat.forPattern("dd/MM/yyyy HH:mm").withZoneUTC().withLocale(Locale.ENGLISH);

    ObjectMapper objectMapper;
    Gexf gexf;
    Graph graph;

    Attribute attBio, attDisplayName, attFollowers, attLocation, attFollowing, attVerified, attStatusesCount
   ...
   public GexfGenerator() {
    ...
    public void addTweet(JsonNode tweet) {
    ...
    private void convertFromTweep(ObjectMapper mapper, ObjectNode tweet) {
        //TODO: seems tweet.put is deprecated, here is an alternative
        // https://stackoverflow.com/questions/27784312/how-serialize-an-object-as-a-value-to-objectnode-in-jackson
        ObjectNode userNode = mapper.createObjectNode();
 ...
 **/
    ...
    /**
     * After graph being build apply edge weights and trim option, if being set
     */
    void postProcess() {
        if (edgeWeight != null) {
    ...
    /**
     * Post process the graph, i.e. apply weights and then trim it, if it is being asked,
     * then print the graph as gexf file
     */
    void writeToFile() {
        postProcess()
